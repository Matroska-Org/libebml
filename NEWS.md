...
* Fixed several problems in EbmlBinary: potential segfault by writing
  to a nullptr, potential memory leak under certain error conditions.

# Version v1.4.2 2021-02-18

* Fixed several heap overflow bugs in the `ReadData` functions of
  various data type classes. This fixes CVE-2021-3405.

# Version v1.4.1 2021-01-04

* Fixed a case EbmlMaster::Read where the element returned via
  UpperEltFound and FountElt points to a just-deleted element, causing
  callers to think the memory returned is valid, potentially leading
  to use-after-free/double-free errors. This can happen if the
  specific element's Read function throws an exception when
  encountering certain invalid data constellations.

# Versiond v1.4.0 2020-06-27
...
* Fixed the EbmlMaster::GetDataStart() function returning wrong values
  for elements with an infinite/unknown size.
...
# Versiond v1.3.4 2016-07-02

* EbmlVersion.cpp: in order to enable deterministic builds the
  EbmlCodeDate variable has been set to "Unknown" instead of the date
...
* EbmlBinary.h: add #include <cstdlib> for compilation with clang and
  libc++. Patch by Thomas Klausner <wiz@NetBSD.org>.

# Versiond v1.3.3 2015-10-20
...
# Versiond v1.3.2 2015-10-17

* EbmlElement::FindNextElement(): Handle EOF when reading the element
  size properly.
...
* build system: switched the build system from hand-crafted Makefiles
  to an autoconf/automake-based system. A pkg-config file will be
  installed as well; its name is »libebml«. Patch by Jan Engelhardt
  <jengelh@inai.de>.
...
* EbmlMemoryStream: add a new class for safe memory access that throws
...
* GetValue(), SetValue(), GetValueUTF8(), SetValueUTF8() functions for
  EbmlString/EbmlUnicodeString
* Date values use 64bit integers
* Integer conversion operators were made 'const'
* GetElementList() function for EbmlMaster providing a reference to
  the underlying std::vector<> for better interaction with the C++
  standard library
...
* fix the constructor of CRTError in StdIOCallback (patch from Graham
  'gbooker' Booker)
* fix the size returned by MemIOCallback::read() when reaching the end
  of the buffer
